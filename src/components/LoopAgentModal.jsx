...
export function LoopAgentModal({ isOpen, onClose }) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="bg-slate-50 dark:bg-gradient-to-br dark:from-slate-900 dark:to-gray-900 text-gray-900 dark:text-gray-100 sm:max-w-xl rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700/50 p-0 overflow-hidden transition-all duration-300 ease-in-out flex flex-col max-h-[90vh] sm:max-h-[85vh]"> {/* Added flex flex-col and max-h for overall modal height constraint */}

        {/* Main padded container - this div itself does not scroll its direct children */}
        {/* The p-0 on DialogContent and then padding here is intentional for the new structure */}

        {/* Header Section (Fixed) */}
        <div className="p-6 sm:p-8 pb-4 sm:pb-6 relative border-b border-slate-200 dark:border-slate-700/50 flex-shrink-0"> {/* Added flex-shrink-0 and bottom border */}
          <button
            onClick={onClose}
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center text-center sm:text-left">
        ...
        {/* Added px-6 sm:px-8 for horizontal padding, pt-4 sm:pt-6 for top padding, pb-6 sm:pb-8 for bottom padding */}
        <div className="p-6 sm:p-8 pt-4 sm:pt-6 border-t border-slate-200 dark:border-slate-700/50 flex-shrink-0"> {/* Added flex-shrink-0 and top border */}
